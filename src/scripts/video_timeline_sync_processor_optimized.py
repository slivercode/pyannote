...
class OptimizedVideoTimelineSyncProcessor:
    ...
    def __init__(
            ...
            use_gpu: bool = False,  # 默认禁用GPU，需要明确指定True才启用
        ...
        Args:
            ...
            use_gpu: 是否使用GPU加速（False=禁用[默认]，True=启用并检测）
            quality_preset: 质量预设
    ...
    def _configure_gpu(self, use_gpu: Optional[bool]) -> bool:
        ...
        Args:
            use_gpu: 用户指定的GPU设置（False=禁用[默认]，True=启用并检测）

        Returns:
            是否启用GPU加速
        """
        # 默认禁用GPU，只有明确指定 use_gpu=True 时才启用
        if not use_gpu:
            # 默认使用CPU处理（不进行GPU检测，节省启动时间）
            print("\n💻 使用CPU处理模式")
            self.gpu_caps = {
                'has_nvidia': False,
                'has_cuda': False,
...
if __name__ == "__main__":
    ...
    processor = OptimizedVideoTimelineSyncProcessor(
        ...
        use_gpu=False,  # 默认CPU模式（False=禁用，True=启用GPU）
